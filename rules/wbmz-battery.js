...
function updatePowerStatus(newCurrent) {
  if (lastTriggeredCurrent != null)
    if (Math.abs(newCurrent - lastTriggeredCurrent) < currentHysteresis) return;
...
function readI2cData() {
  /*Читаем данные*/
  runShellCommand(
    "i2cdump -y -r 0x01-0x0C {} 0x70 c | grep 00: | sed -e 's/00: //g' -e 's/    .*//g'".format(
      config.bus
    ),
    {
      captureOutput: true,
      exitCallback: function (exitCode, capturedOutput) {
        if (!capturedOutput) {
          ...
          dev['battery']['Percentage'] = Math.round(
            (charge_mah - wbmz2_ps.correction) / (wbmz2_ps.batteryСapacity / 100)
          );
        }
      },
    }
